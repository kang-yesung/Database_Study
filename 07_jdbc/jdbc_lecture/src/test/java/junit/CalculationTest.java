...
class CalculationTest {
    ...
    @Test //테스트 메서드를 의미하고, Junit이 이메서드를 테스트 대상으로 인식
    @DisplayName("더하기 테스트")
        // 테스트 실행할때 표시될 이름을 지정
    void additionTest() {
        int result = 2 + 3;
        assertEquals(5, result); //예상값이 실제값과 동일한지확인
    }
    ...
    @Test
        //@Disabled("아직구현되지않은") // 테스트를 임시로 비활성화
    void add() {
        // given : 테스트에 필요한 준비
    ...
    * Junit과 함께 사용되어 테스트 코드에서좀더 쉽고 표현력있는검증문법 사용 가능
    * */
    @Test
